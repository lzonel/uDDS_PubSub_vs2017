...
void UserDataTypeListener::on_data_available(DataReader* reader)
{
	...
	for (i = 0; i < data_seq.length() && (nums < all_nums || all_nums == 0); ++i) {
		UserDataType_writer->write(data_seq[i], instance_handle);
		std::cout << "pub-->sub sent:" << nums << std::endl;
		nums++;
	}
}
...
/* 建议2：一个participant下创建一个subscriber即可，无需重复创建 */
	...
	/* 7. 主循环 ，发送数据 */
	while (nums < all_nums || all_nums == 0) {
	...
	auto elapsed_micros = std::chrono::duration_cast<std::chrono::microseconds>(elapsed);

	std::cout << "时延: " << elapsed_micros.count() / all_nums / 2 << " microseconds" << std::endl;
