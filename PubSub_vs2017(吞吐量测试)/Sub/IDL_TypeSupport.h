...
public:
	...
	static void print_data(UserDataType* pUserDataType, int layer = 0)
	{
		...
		std::cout << c << "  md5 = " << pUserDataType->get_md5() << std::endl;
		if (0 == pUserDataType->check_data_md5())
		{
			std::cout << c << "  check_md5 = " << "MD5校验出错!" << std::endl;

		}
		else
		{
			std::cout << c << "  check_md5 = " << "MD5校验成功!" << std::endl;
		}
