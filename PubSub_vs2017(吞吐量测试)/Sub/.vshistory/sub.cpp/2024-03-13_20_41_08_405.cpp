...
void calc_Mbps() {
	...
	{
		lock.unlock();
		double packetLossRate = static_cast<double>(usi_rcv_all - usi_rcv_succ) / usi_rcv_all * 100;
		std::cout << "Recv IO: " << usi_data_size * 8 << " Mbps " << " 丢包率: " << packetLossRate << "%" << std::endl;//也可能是我理解错了,贵公司给的Mbps算法,不适用data长度来回变动的情况,所以我擅自使用常规的想法
		usi_data_size = 0;
		usi_rcv_succ = 0;
		usi_rcv_all = 0;
		lock.lock();
