...
std::mutex mutx_Mbps;
int usi_data_size = 0;
int usi_rcv_succ = 0;//代表成功接收次数
int usi_rcv_succ = 0;//代表总接收次数
void calc_Mbps() {
	...
	{
		lock.unlock();
		std::cout << "Recv IO: " << usi_data_size * 8 << " Mbps" << std::endl;//也可能是我理解错了,贵公司给的Mbps算法,不适用data长度来回变动的情况,所以我擅自使用常规的想法
		usi_data_size = 0;
		lock.lock();
